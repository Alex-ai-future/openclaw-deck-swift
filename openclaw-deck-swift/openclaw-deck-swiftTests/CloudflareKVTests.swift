...
final class CloudflareKVTests: XCTestCase {
  ...
  func testConfigure_withValidURL() {
    let validURL = "https://api.cloudflare.com/client/v4/accounts/test-account/storage/kv/namespaces/test-namespace"
  ...
  func testConcurrentConfiguration() {
    ...
    for i in 0..<10 {
      Task {
        cloudflare.configure(
          url: "https://api.cloudflare.com/client/v4/accounts/test\(i)/storage/kv/namespaces/test\(i)",
          apiToken: "token\(i)"
        )
        if i == 9 {
          expectation.fulfill()
