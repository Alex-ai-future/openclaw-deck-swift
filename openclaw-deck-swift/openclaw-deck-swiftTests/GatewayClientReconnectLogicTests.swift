...
final class GatewayClientReconnectLogicTests: XCTestCase {
    ...
    func testReconnect_networkJitter_successAfterFirstAttempt() async {
    ...
    func testReconnect_networkJitter_preservesState() async {
        let mockWebSocket = MockWebSocketConnection()
        let client = GatewayClient(
            url: URL(string: "ws://localhost:8080")!,
            token: nil,
    ...
    func testReconnect_maxAttempts_stopsAfter10Failures() async {
        let mockWebSocket = MockWebSocketConnection()
        let client = GatewayClient(
            url: URL(string: "ws://localhost:8080")!,
            token: nil,
    ...
    func testReconnect_exponentialBackoff_delayIncreases() async {
        let client = GatewayClient(
            url: URL(string: "ws://localhost:8080")!,
            token: nil
        )
    ...
    func testReconnect_backoffCapsAtMaxDelay() async {
        let client = GatewayClient(
            url: URL(string: "ws://localhost:8080")!,
            token: nil
        )
    ...
    func testManualDisconnect_cancelsAutoReconnect() async {
        let mockWebSocket = MockWebSocketConnection()
        let client = GatewayClient(
            url: URL(string: "ws://localhost:8080")!,
            token: nil,
    ...
    func testAutoReconnect_doesNotNotifyUI() async {
        let mockWebSocket = MockWebSocketConnection()
        let client = GatewayClient(
            url: URL(string: "ws://localhost:8080")!,
            token: nil,
        ...
        // 初始连接 - 触发回调
        await client.connect()
        XCTAssertEqual(connectionCallbacks.filter { $0 }.count, 1, "初始连接应该触发回调")
        ...
        // 验证：自动重连不触发 UI 回调
        XCTAssertEqual(connectionCallbacks.filter { $0 }.count, 1, "自动重连不应该触发 UI 回调")
    }
    
    // MARK: - 重连状态机测试
    
    func testReconnectStateMachine_consecutiveFailures() async {
        let client = GatewayClient(
            url: URL(string: "ws://localhost:8080")!,
            token: nil
        )
    ...
    func testReconnectStateMachine_successResetsEverything() async {
        let mockWebSocket = MockWebSocketConnection()
        let client = GatewayClient(
            url: URL(string: "ws://localhost:8080")!,
            token: nil,
    ...
    func testReconnect_duplicateCalls_skipsSecond() async {
        let mockWebSocket = MockWebSocketConnection()
        let client = GatewayClient(
            url: URL(string: "ws://localhost:8080")!,
            token: nil,
    ...
    func testReconnectConfiguration_defaults() {
        let client = GatewayClient(
            url: URL(string: "ws://localhost:8080")!,
            token: nil
        )
